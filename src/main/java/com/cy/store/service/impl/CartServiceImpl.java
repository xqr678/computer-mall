...
public class CartServiceImpl implements ICartService {
    ...
    public Integer reduceNum(Integer cid, Integer uid, String username) {
        ...
        if (num>0){
            ...
            // 调用updateNumByCid(cid, num, modifiedUser, modifiedTime)执行修改数量
             rows = cartMapper.updateNumByCid(cid, num, username, now);
            if (rows != 1) {
                throw new InsertException("修改商品数量时出现未知错误，请联系系统管理员");
            }
            return num;
        }else {
             rows = cartMapper.updateNumByCid(cid,0, username, now);
             return 0;
        }
        // 返回新的数量
    ...
    @Override
    public Integer deleteByUidAndCid(Integer[] cids) {
        Integer rows=cartMapper.deleteByUidAndCid(cids);
        return rows;
    }

    @Override
    public List<OrderItem> findByOids(Integer[] oids) {
        List<OrderItem> oidsList = cartMapper.findByOids(oids);
        return oidsList;
    }
}
